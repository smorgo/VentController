...
namespace MqttClient.Core
{
    public class StreamFactory
    {
        ...
        public static INetworkStream CreateStream(string connStr)
        {
        ...
        private static INetworkStream CreateTcpStreams( Hashtable parms )
        {
            string host = (string)parms[PARAM_HOST];
            int port = Convert.ToInt32(parms[PARAM_PORT]);

			var tcpclnt = DependencyService.Get<ITcpClient> (DependencyFetchTarget.NewInstance);
            try
            {
